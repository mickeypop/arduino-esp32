...
// If you want; add a button press, condition etc... to run initSmartConfig() 
// any time you want to move to a different Access Point.

// I left notes to where i used an RGB flasher if you want to add state indicators.
// ESP32Ticker library set the timing for flashing leds if you want a good esp32 ticker.
// My approach to the flasher was to leave the ticker running on the second core and set a 
// variable true or false to start and stop the LED.

// by github @mickeypop , contact me if you would like the flasher code too

#include "FS.h"
#include "esp_system.h"  // needed for some typedefs and structs
#include <esp_wifi.h>
#include <string.h>
#include <WiFi.h>
  ...
  {            // WiFi DOWN
     ...
     while (WiFi.status() != WL_CONNECTED && WLcount < 200 )
     {
      delay( 100 );
         ...
         if (UpCount >= 60)  // keeps from scrolling sideways forever
         {
            UpCount = 0;
            Serial.printf("\n");
         }
...
void wifiInit()  // 
{
   ...
   {  			// not the same, setup with SmartConfig
      if( PrefSSID == "none" )  // New...setup wifi run SmartConfig
      {
        initSmartConfig(); 
        delay( 3000);
   ...
   // I flashed LEDs while connecting
...
void initSmartConfig() 
{
  ...
  while ( !WiFi.smartConfigDone() ) 
  {
     // flash led to indicate not configured
        Serial.printf( "." );
     if( loopCounter >= 40 )  // keeps from scrolling sideways forever
     {
         loopCounter = 0;
         Serial.printf( "\n" );
     }
